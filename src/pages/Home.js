...
const BlackHoleName = ({ text1, text2 }) => {
  ...
  const getPseudoRand = (seed) => {
    const x = Math.sin(seed++) * 1e4;
    return x - Math.floor(x);
  };
  return (
    <div
      className="relative inline-block cursor-crosshair group mb-3 z-20 w-fit mx-auto lg:mx-0"
      onMouseEnter={() => setIsHovering(true)}
      onMouseLeave={() => setIsHovering(false)}
    >
      <motion.div
        className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 rounded-full mix-blend-screen pointer-events-none z-30"
        initial={{ width: 0, height: 0, opacity: 0 }}
        animate={isHovering ? {
          width: ["0px", "10vw", "40vw", "0px"],
          height: ["0px", "10vw", "40vw", "0px"],
          opacity: [0, 1, 1, 0],
          boxShadow: [
            "0 0 0px 0px rgba(0, 245, 255, 0)",
            "0 0 50px 20px rgba(0, 245, 255, 0.8), inset 0 0 20px 10px rgba(0,0,0,1)",
            "0 0 100px 40px rgba(147, 51, 234, 0.6), inset 0 0 50px 20px rgba(0,0,0,1)",
            "0 0 0px 0px rgba(0, 245, 255, 0)"
          ],
          backgroundColor: ["transparent", "#000", "#000", "transparent"]
        } : {
          width: 0,
          height: 0,
          opacity: 0,
          boxShadow: "0 0 0px 0px rgba(0, 245, 255, 0)",
      ...
      <motion.div
        className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 rounded-full w-[40vw] h-[40vw] max-w-[250px] max-h-[250px] pointer-events-none bg-brand-cyan/40 blur-[60px] z-[0]"
        animate={{
          opacity: isHovering ? [0, 1, 0] : 0,
          scale: isHovering ? [0.5, 1.2, 3] : 1
        }}
        transition={{ duration: 3.5 }}
      />

      <h1 className="font-display text-[clamp(2.5rem,8vw,5rem)] font-black text-white leading-tight drop-shadow-[0_0_40px_rgba(0,245,255,0.3)] pointer-events-none text-center lg:text-left">
        <span className="inline-block whitespace-nowrap">
          {text1.split("").map((char, i) => {
            const centerDistX = i - text1.length / 2;
            const r1 = (getPseudoRand(i + 1) - 0.5) * 300;
            const r2 = (getPseudoRand(i + 14) - 0.5) * 200;
            const rRot = (getPseudoRand(i + 33) - 0.5) * 720;
            return (
              <motion.span
                key={i}
                className="inline-block origin-center relative z-10"
                animate={isHovering ? {
                  x: [0, -centerDistX * 20, r1, 0],
                  y: [0, 45, r2, 0],
                  scale: [1, 0.1, 1.5, 1],
                  rotate: [0, centerDistX * 45, rRot, 0],
                  opacity: [1, 0.3, 0.9, 1],
                  filter: ["blur(0px)", "blur(10px)", "blur(2px)", "blur(0px)"],
                  color: ["#ffffff", "var(--color-brand-cyan)", "var(--color-brand-purple)", "#ffffff"],
                  textShadow: ["0 0 0px transparent", "0 0 20px var(--color-brand-cyan)", "0 0 20px var(--color-brand-purple)", "0 0 0px transparent"]
                } : {
                  x: 0,
                  y: 0,
                  scale: 1,
        ...
        <span className="inline-block whitespace-nowrap">
          {text2.split("").map((char, i) => {
            const centerDistX = i - text2.length / 2;
            const r1 = (getPseudoRand(i + 51) - 0.5) * 300;
            const r2 = (getPseudoRand(i + 74) - 0.5) * 200;
            const rRot = (getPseudoRand(i + 93) - 0.5) * 720;
            return (
              <motion.span
                key={i}
                className="inline-block origin-center relative z-10"
