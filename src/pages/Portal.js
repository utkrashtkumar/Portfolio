...
import { Lock, Unlock, Mail, ShieldAlert, CheckCircle2, RotateCw } from 'lucide-react';
...
const AuthPortal = ({ onAuthSuccess }) => {
  const [view, setView] = useState('signin'); // signin | signup | configure | verify
  ...
  const handleAuth = async (e) => {
    ...
    try {
      if (view === 'signin') {
        ...
        if (error) {
          // If Supabase flags that email is unconfirmed
  ...
  const handleResend = async () => {
    ...
    await new Promise(r => setTimeout(r, 600));
    try {
      // Supabase signup triggers resends implicitly for unconfirmed credentials
      const { error } = await supabase.auth.signUp({ email, password: password || "tempPasswordForResend123!" });
  ...
  if (!config.isConfigured && view !== 'configure') {
    setView('configure');
  }
  ...
  return (
    <div className="glass-panel border border-brand-cyan/25 p-6 max-w-sm mx-auto font-mono text-xs text-slate-300 space-y-4 shadow-xl">
      <div className="border-b border-white/5 pb-3 flex items-center justify-between">
        ...
        {config.isConfigured && view !== 'verify' && (
          <button 
            type="button"
            onClick={() => {
      ...
      {view === 'configure' ? (
        <div className="space-y-3 animate-fadeIn">
          <div className="bg-yellow-500/10 border border-yellow-500/20 p-2.5 rounded text-[0.58rem] text-yellow-200 leading-normal">
            <strong>⚙ GATEWAY CONFIG:</strong> Configure your authentication endpoint and key to enable Sign In / Sign Up modules. URL credentials will store securely in localStorage.
          </div>
          <div className="space-y-2">
            <div>
              <label className="text-[0.52rem] text-slate-500 block mb-1 uppercase">Auth API URL</label>
              <input
                type="text"
                placeholder="https://xyz.auth-endpoint.co"
                value={supabaseUrlInput}
                onChange={e => setSupabaseUrlInput(e.target.value)}
                className="w-full px-3 py-1.5 bg-black/40 border border-white/10 rounded font-mono text-[0.62rem] text-white outline-none focus:border-brand-cyan/40"
              />
            </div>
            <div>
              <label className="text-[0.52rem] text-slate-500 block mb-1 uppercase">Auth Anon Key</label>
              <input
                type="text"
            ...
            <button
              ...
              className="w-full py-2 bg-brand-cyan/20 border border-brand-cyan/35 text-brand-cyan rounded uppercase font-bold tracking-wider cursor-pointer hover:bg-brand-cyan/30 active:scale-95 transition-all"
            >
              Save Configuration
            </button>
            {config.isConfigured && (
              <button
                ...
                className="w-full py-1.5 bg-white/5 border border-white/10 text-slate-400 rounded uppercase cursor-pointer mt-1"
              >
                Back
              </button>
            )}
        ...
        <div className="space-y-4 animate-fadeIn">
          ...
          {logs.length > 0 && (
            <div className="bg-black/80 p-2.5 rounded border border-white/5 text-[0.55rem] text-slate-500 space-y-0.5">
              {logs.map((l, i) => <div key={i} className={l.startsWith('[SUCCESS]') ? 'text-brand-green font-bold' : ''}>{l}</div>)}
            </div>
          )}
        ...
        <form onSubmit={handleAuth} className="space-y-3.5 animate-fadeIn">
          <div className="space-y-2.5">
            <div>
              <label className="text-[0.52rem] text-slate-500 block mb-1 uppercase">Email Address</label>
              <input
                type="email"
                required
                value={email}
                ...
                className="w-full px-3 py-2 bg-black/40 border border-white/10 rounded text-white outline-none focus:border-brand-cyan/40"
              />
            </div>
            <div>
              <label className="text-[0.52rem] text-slate-500 block mb-1 uppercase">Access Code (Password)</label>
              <input
                type="password"
                required
                value={password}
            ...
            {view === 'signup' && (
              <div>
                <label className="text-[0.52rem] text-slate-500 block mb-1 uppercase">Confirm Password</label>
                <input
                  type="password"
                  required
          ...
          <div className="pt-2.5 border-t border-white/5 flex justify-between text-[0.52rem] text-slate-500 select-none animate-fadeIn">
            <span>Server: Core Auth API</span>
            <button 
              type="button"
              onClick={() => setView('configure')} 
              className="text-slate-500 hover:text-brand-cyan uppercase bg-transparent border-none cursor-pointer"
            >
              Settings
            </button>
          </div>
